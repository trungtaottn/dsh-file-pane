 ...
 */
import { readFileResult, listDir, diffSides, renderMarkdown, resolveWithin, ViewError } from "./view-core.mjs";
...
export function apply(ctx, config = {}) {
	...
	ctx.webServer.register({
		...
		handler: async (req, res) => {
			try {
				const url = req.url ?? "/";
				const { path: rel, raw, diff, session, mode, embed, json: asJson, workspace, branch, q, case: caseParam, globs, max, dir } = parseQuery(url);
				...
				if (rel) {
					try {
						...
						res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" });
						return void res.end(paneFileHTML(file, emb));
					} catch (err) {
