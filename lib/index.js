...
/**
 * Same-origin gate for mutating POST routes (defense-in-depth against
 * cross-site request forgery). Browsers attach `Origin` to POSTs; a present
 * Origin must match the Host authority. Requests without Origin (curl/CLI,
 * same-origin fetch with no-cors) are allowed — they cannot be forged by a
 * cross-site page.
 */
function isSameOriginPost(req) {
	const origin = req.headers?.origin;
	if (origin === undefined || origin === null) return true; // no Origin → not browser-forgeable
		...
		handler: async (req, res) => {
			try {
				...
				if (req.method === "POST" && pathname.endsWith("/api/spill")) {
					...
					if (!isSameOriginPost(req)) return plain(res, 403, "forbidden: cross-site post");
				...
				if (req.method === "POST" && pathname === "/browser/api/git/checkout") {
					if (!isSameOriginPost(req)) return json(res, 403, { ok: false, error: "forbidden: cross-site post" });
					const git = await isGitRepo(ws);
