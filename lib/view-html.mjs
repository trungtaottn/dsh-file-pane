 ...
 * numbers emitted absolute (start + i + 1). Returns the grid rows HTML.
 */
export async function renderTextRows(lines, name, { start = 0, settings } = {}) {
	const lang = langForFile(name);
	const cells = await Promise.all(lines.map((l) => lineCell(l, lang, settings)));
	const rows = [];
	for (let i = 0; i < cells.length; i++) {
		rows.push(`<div class="no">${start + i + 1}</div><div class="code" data-ln="${start + i + 1}">${cells[i]}</div>`);
	}
	return rows.join("");
}
...
function toggleMd(){
	...
	if(b.style.display==='none'){b.style.display='block';r.style.display='none';t.dataset.on='1';t.textContent='raw';}
		...
		const rows = await renderTextRows(lines, name, { start, settings });
...
<div id="mdRaw" class="archive" style="display:none">${rows}</div>
			...
			main = `<div class="fcol">${filemeta}<div class="main" style="padding-top:10px"><div class="archive" data-total="${total}"${large}>${rows}</div></div></div>`;
		}
	} else {
