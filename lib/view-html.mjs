...
import { langForFile, highlightLine } from "./highlight.mjs";
...
/**
 * Defense-in-depth whitelist for Shiki token HTML. Shiki escapes code text by
 * construction, but this strips anything that is not a bare `span`/`pre`/`code`
 * tag and any `on*=` attribute, so a future regression can never inject a
 * handler or foreign tag. Returns the input unchanged when it contains no such
 * constructs (the normal case).
 */
export function sanitizeTokenHtml(html) {
	if (typeof html !== "string" || !html) return html ?? "";
	// Remove any tag not in the allowlist (closing or opening).
	let out = html.replace(/<\/(?!span|pre|code\b)[a-zA-Z0-9]+[^>]*>/g, "");
	out = out.replace(/<(?!\/?(?:span|pre|code)\b)[a-zA-Z0-9][^>]*>/g, "");
	// Strip on*=/javascript:/style= attributes (defense against attr injection).
	out = out.replace(/\son\w+\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, "");
	out = out.replace(/\s(?:href|src)\s*=\s*(")?\s*javascript:[^">]*(")?/gi, "");
	return out;
}

/** Highlight a line (async) or fall back to escaped plain text. */async function lineCell(line, lang, settings) {
	if (!lang) return `<span class="tok-cell">${esc(line)}</span>`;
	try {
		const html = await highlightLine(line, lang, settings?.enabledLangs);
		if (html) return `<span class="tok-cell">${sanitizeTokenHtml(html)}</span>`;
	} catch { /* fall through */ }
	return `<span class="tok-cell">${esc(line)}</span>`;
}

/**
 * Render text rows (async) with Shiki token spans when a grammar is known.
 * `slice` = { start } (0-based window start) for Phase-3 windowing; line
 * numbers emitted absolute (start + i + 1). Optional `blame` = { lines: [] }
 * from the git blame cache adds a `.blame` annotation cell per line. Returns
 * the grid rows HTML.
 */
export async function renderTextRows(lines, name, { start = 0, settings, blame } = {}) {
	const lang = langForFile(name);
	const cells = await Promise.all(lines.map((l) => lineCell(l, lang, settings)));
	const rows = [];
	const blameLines = blame?.lines ?? null;
	for (let i = 0; i < cells.length; i++) {
		const b = blameLines ? blameLines[start + i] : null;
		const blameCell = blameLines
			? `<div class="blame" title="${b ? esc(`${b.sha} · ${b.author}`) : ""}">${b ? esc(`${(b.sha ?? "").slice(0, 7)} ${b.author ?? ""}`) : ""}</div>`
			: "";
		rows.push(`<div class="no">${start + i + 1}</div>${blameCell}<div class="code" data-ln="${start + i + 1}">${cells[i]}</div>`);
	}
	return rows.join("");
}
...
/* themed slim scrollbars — default native scrollbars clash with the dark bg */
*{scrollbar-width:thin;scrollbar-color:var(--border2) transparent}
*::-webkit-scrollbar{width:10px;height:10px}
*::-webkit-scrollbar-track{background:transparent}
*::-webkit-scrollbar-thumb{background:var(--border2);border-radius:6px;border:2px solid var(--bg)}
*::-webkit-scrollbar-thumb:hover{background:var(--accent);border-color:var(--panel)}
...
/* embed mode (in-app dock iframe): no chrome, content fills the pane */
body[data-embed]{height:100%}
body[data-embed] .bar,body[data-embed] .statusbar,body[data-embed] .diffwrap + .diffempty{display:none}
body[data-embed] .main{padding:12px 16px}
body[data-embed] .body{flex:1}
...
.main{flex:1;overflow:auto;padding:18px 24px;min-width:0;max-width:100%}
.dirlistwrap{flex:1;min-width:0;overflow:auto}
.dirlist{display:flex;flex-direction:column;padding:8px}
.diritem{display:flex;align-items:center;gap:8px;padding:5px 10px;border-radius:6px;color:var(--fg-dim);text-decoration:none;font-family:var(--mono);font-size:13px;flex:none}
.diritem:hover{background:var(--panel2);color:var(--fg)}
.diritem .ic{width:15px;flex:none;display:inline-flex;justify-content:center;color:var(--faint)}
.diritem:hover .ic{color:var(--muted)}
...
/* text archive (file view) — pre-wrap so content reflows/resizes with the pane */
.archive{display:grid;grid-template-columns:auto minmax(0,1fr);grid-auto-rows:minmax(0,auto);font-family:var(--mono);font-size:13px;gap:0 12px;line-height:1.55}
.archive.has-blame{grid-template-columns:auto auto minmax(0,1fr);gap:0 10px}
...
.archive .blame{color:var(--faint);font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:190px;user-select:none;padding-right:8px}
.archive .code{white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;padding-right:4px}
/* Shiki dual-theme token spans: color follows the environment (dark by default,
   light when prefers-color-scheme flips). Only CSS vars — no hardcoded colors,
   so the theme-switcher plan can flip this layer without edits here. */
.archive .code .tok-cell span{color:var(--shiki-light,#cdd5e4)}
@media (prefers-color-scheme:dark){.archive .code .tok-cell span{color:var(--shiki-dark,#eef1f8)}}
.archive .code .tok-cell{color:var(--fg-dim)}
/* Large-file zero-dep fallback: content-visibility bounds render cost while
   keeping .no gutters (sticky) outside the contain. */
body[data-large] .archive > .code{content-visibility:auto;contain-intrinsic-size:auto 20px}
...
.fcol{flex:1;min-width:0;max-width:100%;min-height:0;display:flex;flex-direction:column}
...
.diffwrap{flex:1;min-width:0;min-height:0;overflow:auto;font-family:var(--mono);font-size:13px;line-height:1.55;background:var(--bg)}
.dview{display:none}
...
/* git-style unified diff (---/+++ headers, @@ hunks, +/- lines, no line-number column) */
.ghead{font-family:var(--mono);font-size:12px;color:var(--muted);padding:3px 12px;background:var(--panel);border-bottom:1px solid var(--border)}
.gdline{display:flex;align-items:stretch}
.gdline .gsg{width:16px;flex:none;user-select:none;font-weight:700;color:var(--muted);text-align:center}
.gdline .gtx{white-space:pre;flex:1;min-width:0;padding:0 10px;color:var(--fg-dim)}
.gdline.ctx .gtx{color:var(--fg-dim)}
.gdline.add{background:var(--add-bg)}
.gdline.add .gsg{color:var(--add-line)}
.gdline.add .gtx{color:var(--add-fg)}
.gdline.del{background:var(--del-bg)}
.gdline.del .gsg{color:var(--del-line)}
.gdline.del .gtx{color:var(--del-fg)}
/* intraline change marks (GitHub/VS Code-style inline blocks) */
.inline-del{background:rgba(255,107,107,.28);color:var(--del-fg);border-radius:3px;padding:0 1px}
.inline-add{background:rgba(95,214,135,.25);color:var(--add-fg);border-radius:3px;padding:0 1px}
...
export function paneDirHTML({ path: rel, entries }, embed = false) {
	const items = entries.map((e) => {
		const sub = rel ? rel + "/" + e.name : e.name;
		const icon = e.dir ? "folder" : e.mime?.startsWith("image/") ? "image" : e.mime === "application/pdf" ? "pdf" : "file";
		return `<a class="diritem" href="${bpath(sub)}"><span class="ic">${ic(icon)}</span><span>${esc(e.name)}</span></a>`;
	}).join("");
	const top = embed ? "" : TOPBAR(rel || "/");
	const body = entries.length === 0
		? '<div class="placeholder">( empty )</div>'
		: `<div class="dirlist">${items}</div>`;
	return PAGE_SHELL(rel || "/", `${top}<div class="dirlistwrap">${body}</div>`, "", embed ? " data-embed" : "");
}

export async function paneFileHTML(file, embed = false, opts = {}) {
	const rel = file.path;
	const name = fsb(rel);
...
<a class="act" href="#" data-copy="${q(rel)}" onclick="navigator.clipboard.writeText(decodeURIComponent(this.dataset.copy));this.querySelector('.lb').textContent='copied'">${ic("copy")}<span class="lb">copy</span></a>
	...
	if (file.kind === "image") {
		...
		// native iframe stays as the no-JS / asset-missing fallback. Height is
		// 100% so the viewer fills and resizes with the pane frame (was 78vh,
		// which clipped in the dock and did not track the frame).
		main = `<div class="note">${file.kind} · ${file.size} bytes</div>
<iframe id="pdfNative" src="/browser/?path=${q(rel)}&raw=1" style="width:100%;height:100%"></iframe>
<pdfjs-viewer-element id="pdfViewer" src="/browser/?path=${q(rel)}&raw=1" viewer-css-theme="DARK" style="width:100%;height:100%;display:none"></pdfjs-viewer-element>
<script>
...
function toggleMd(){
	...
	if(b.style.display==='none'){b.style.display='block';r.style.display='none';t.dataset.on='1';t.textContent='raw';}
		...
		// Phase-3 windowing: slice [start, start+lines) when requested; otherwise
		// all lines. `data-large` marks big files for content-visibility CSS.
		const slice = opts.slice ?? null;
		const start = slice?.start ?? 0;
		const lines = slice ? all.slice(start, start + (slice.lines ?? all.length)) : all;
		const settings = opts.settings;
		const rows = await renderTextRows(lines, name, { start, settings, blame: opts.blame });
		const large = !slice && opts.dataLarge ? " data-large" : "";
...
<div id="mdRaw" class="archive" style="display:none">${rows}</div>
			...
			main = `<div class="fcol">${filemeta}<div class="main" style="padding-top:10px"><div class="archive${opts.blame ? " has-blame" : ""}" data-total="${total}"${large}>${rows}</div></div></div>`;
		}
	} else {
...
<div class="mono">${esc(name)}</div>`;
	...
	const top = embed ? "" : bar;
	return PAGE_SHELL(`${name} · dsh-file-pane`, `${top}<div class="body">${main}</div>`, "", `${embed ? " data-embed" : ""}${opts.dataLarge ? " data-large" : ""}`);
}

const HUNK_CTX = 3;

/**
 * Render an intraline segment array (from view-core inlineWords/attachInlineMarks)
 * as escaped spans; changed segments get the inline-del / inline-add overlay.
 * `cls` is the side ("del" or "add") that receives the mark class.
 */
function inlineHtml(segs, cls) {
	if (!Array.isArray(segs) || segs.length === 0) return "";
	const mark = cls === "del" ? "inline-del" : "inline-add";
	return segs
		.map((s) => (s.type === "ch" ? esc(s.text) : `<span class="${mark}">${esc(s.text)}</span>`))
		.join("");
}

/** Render a diff row's text cell honoring optional intraline marks. */
function rowText(r, side) {
	if (side === "old") {
		if (r.inline && r.inline.old) return inlineHtml(r.inline.old, "del");
		return r.kind === "add" ? "" : esc(r.oldText ?? "");
	}
	if (r.inline && r.inline.next) return inlineHtml(r.inline.next, "add");
	return r.kind === "del" ? "" : esc(r.newText ?? "");
}

/** Render aligned rows as a git-style unified diff (---/+++ headers, @@ hunks, +/- lines, no line-number column). */
function gitUnifiedHTML(rows, rel) {
	const out = [`<div class="ghead">--- a/${esc(rel)}</div>`, `<div class="ghead">+++ b/${esc(rel)}</div>`];
	let a = 0;
	const n = rows.length;
	while (a < n) {
		let b = a;
		while (b < n && rows[b].kind === "ctx") b++;
		if (b === n) break;
		const lead = Math.max(a, b - HUNK_CTX);
		let runEnd = b;
		while (runEnd < n && rows[runEnd].kind !== "ctx") runEnd++;
		const trail = Math.min(n, runEnd + HUNK_CTX);
		const hunk = rows.slice(lead, trail);
		let oldC = 0, newC = 0;
		for (const r of hunk) { if (r.kind !== "add") oldC++; if (r.kind !== "del") newC++; }
		const oldStart = hunk.find((r) => r.oldLn !== -1)?.oldLn ?? 1;
		const newStart = hunk.find((r) => r.newLn !== -1)?.newLn ?? 1;
		out.push(`<div class="hunkhdr">@@ -${oldStart},${Math.max(1, oldC)} +${newStart},${Math.max(1, newC)} @@</div>`);
		for (const r of hunk) {
			const sign = r.kind === "del" ? "-" : r.kind === "add" ? "+" : " ";
			out.push(`<div class="gdline ${r.kind}"><span class="gsg">${sign}</span><span class="gtx">${rowText(r, "old")}</span></div>`);
		}
		a = trail;
	}
	return out.join("");
 ...
 */
export function paneDiffHTML(file, mode = "unified", embed = false) {
	const rel = file.path;
	const name = fsb(rel);
	const up = rel.includes("/") ? rel.slice(0, rel.lastIndexOf("/")) : "";
	...
	const note = hasSpill ? "before → after" : "no prior version (showing current content only)";
	const initMode = mode === "sbs" ? "sbs" : "unified";

	// ── unified rows (git-style: no line-number column, @@ hunks, -/+/ context) ──
	const uniRows = gitUnifiedHTML(a.rows ?? [], rel);
	...
	const sbsRows = (a.rows ?? []).map((r) => {
		...
		const oc = rowText(r, "old");
		const nc = rowText(r, "new");
...
<span class="ln">${nl}</span><span class="code r">${nc}</span>
	...
	let body;
	if (!hasSpill) {
		body = `<div class="diffempty">No prior version recorded for this session.<br><span style="font-size:12px;color:var(--faint)">The before-edit is captured when the agent edits a file in the open session; reopen via the produced-file chip (or add &amp;session=&lt;id&gt;) to see the diff.</span></div>`;
	} else if (a.rows?.length) {
		body = `<div class="dview unified">${uniRows}</div>
<div class="dview sbs"><div class="diffgrid">
<div class="colhdr l"><b>OLD</b> · before</div><div class="colhdr gutter"></div><div class="colhdr r"><b>NEW</b> · after</div>
${sbsRows}</div></div>`;
	} else {
		body = `<div class="diffempty">( no changes to compare )</div>`;
	}
...
function toggleDiffMode(){
	...
	return PAGE_SHELL(`${name} · diff · dsh-file-pane`, `${embed ? "" : bar}${head}<div class="diffwrap">${body}</div>${embed ? "" : status}${boot}`, "", ` data-mode="${initMode}"${embed ? " data-embed" : ""}`);
}

/** Build a copyable unified diff text (paths + -/+ lines) from aligned rows. */
function buildDiffText(a, rel) {
	if (!a) return "";
	const lines = [`--- ${rel} (before)`, `+++ ${rel} (after)`];
...
/**
 * Per-commit diff page (?gitview=commit): a slim commit header (sha, author,
 * date, subject, body) above the same diff body/cells as paneDiffHTML
 * (gitUnifiedHTML + side-by-side rows, .dline/.gdline/.drow CSS). Reuses
 * counts from the aligned rows.
 */
export function paneCommitHTML(file, mode = "unified", embed = false) {
	const rel = file.path;
	const a = file.aligned ?? { rows: [], added: 0, removed: 0, files: 1 };
	const m = file.meta;
	const initMode = mode === "sbs" ? "sbs" : "unified";
	const uniRows = gitUnifiedHTML(a.rows, rel);
	const sbsRows = (a.rows ?? []).map((r) => {
...
<span class="ln">${nl}</span><span class="code r">${nc}</span>
	...
	const commitHead = m
		? `<div class="commithdr">
<div class="ch-title">${esc(m.subject ?? "")}</div>
<div class="ch-meta"><span class="ch-sha">${esc((m.sha ?? "").slice(0, 7))}</span> · ${esc(m.author ?? "")} · ${esc(m.date ?? "")}</div>
${m.body ? `<div class="ch-body">${esc(m.body)}</div>` : ""}
</div>`
		: "";

	const body = a.rows?.length
		? `<div class="dview unified">${uniRows}</div>
<div class="dview sbs"><div class="diffgrid">
<div class="colhdr l"><b>OLD</b> · ${esc((m?.sha ?? "").slice(0, 7))}^</div><div class="colhdr gutter"></div><div class="colhdr r"><b>NEW</b> · ${esc((m?.sha ?? "").slice(0, 7))}</div>
${sbsRows}</div></div>`
		: `<div class="diffempty">( no changes to compare )</div>`;

	const stats = `<span class="badge add">+${a.added ?? 0}</span><span class="badge del">−${a.removed ?? 0}</span>`;
	const bar = `<div class="bar">
<span class="brand"><span class="logo">${ic("diff")}</span>dsh-file-pane</span>
<span class="crumb">${esc(rel)} @ ${esc((m?.sha ?? "").slice(0, 7))}</span>
<div class="sp"></div>
</div>`;
...
function toggleDiffMode(){
	...
	return PAGE_SHELL(`${fsb(rel)} · commit · dsh-file-pane`, `${embed ? "" : bar}${commitHead}<div class="diffwrap">${body}</div>${embed ? "" : status}${boot}`, "", ` data-mode="${initMode}"${embed ? " data-embed" : ""}`);
}
