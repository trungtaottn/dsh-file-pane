...
export function paneFileHTML(file, embed = false) {
	const rel = file.path;
	const name = fsb(rel);
...
<div class="mono">${esc(name)}</div>`;
	...
	const top = embed ? "" : bar;
	return PAGE_SHELL(`${name} · dsh-file-pane`, `${top}<div class="body">${main}</div>`, "", embed ? " data-embed" : "");
}
...
function gitUnifiedHTML(rows, rel) {
	...
	while (a < n) {
		...
		for (const r of hunk) {
			...
			const txt = r.kind === "add" ? (r.newText ?? "") : (r.oldText ?? "");
			out.push(`<div class="gdline ${r.kind}"><span class="gsg">${sign}</span><span class="gtx">${esc(txt)}</span></div>`);
		}
		a = trail;
	}
	return out.join("");
