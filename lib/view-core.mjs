...
export function mimeFor(name) { return MIME[path.extname(name).toLowerCase()] ?? "application/octet-stream"; }
...
export async function readFileResult(root, rel, opts = {}) {
	...
	const kind = isImage(mime) ? "image" : isPdf(mime) ? "pdf" : isDocx(mime) ? "docx" : isText(mime) ? "text" : "binary";
